...
export async function POST(request: NextRequest) {
  try {
    await connectDB();
    const { point, walletAddress } = await request.json();

    if (point === undefined) {
      return NextResponse.json({ error: 'Point required' }, { status: 400 });
    }
    ...
    const isMine = currentGame.grid[point] === 1;
    ...
    console.error('Mine bet API error:', error);
    return NextResponse.json(
