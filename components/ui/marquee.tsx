...
function MarqueeEdge(props: MarqueeEdgeProps) {
  const { side, size, className, asChild, color, style, ...edgeProps } = props;
  ...
  const gradientStyle = React.useMemo(() => {
    ...
    return {
      ...style,
      backgroundImage: side ? gradientMap[side] : undefined,
    };
  }, [color, side, style]);
  ...
  return (
    <EdgePrimitive
      ...
      style={gradientStyle}
      className={cn(
        marqueeEdgeVariants({ side, size }),
        !color && className,
        color && "pointer-events-none absolute z-10"
      )}
    />
  );
}
